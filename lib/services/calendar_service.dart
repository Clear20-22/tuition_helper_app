...
class CalendarService {
  ...
  List<PaymentModel> getPaymentsForDay(DateTime day) {
    ...
    return payments.where((payment) => isSameDay(payment.dueDate, day)).toList();
  }
  ...
  Map<DateTime, List<dynamic>> getEventMarkersForMonth(DateTime month) {
    ...
    for (final payment in payments) {
      final day = DateTime(payment.dueDate.year, payment.dueDate.month, payment.dueDate.day);
  ...
  Map<String, dynamic> getWeekSummary(DateTime weekStart) {
    ...
    final payments = _storageService.getAllPayments().where((payment) {
      return payment.dueDate.isAfter(weekStart.subtract(const Duration(days: 1))) &&
  ...
  Map<String, dynamic> getMonthSummary(DateTime month) {
    ...
    final sessions = getSessionsForDateRange(startOfMonth, endOfMonth);
    final payments = _storageService.getAllPayments().where((payment) {
      return payment.dueDate.isAfter(startOfMonth.subtract(const Duration(days: 1))) &&
  ...
  List<dynamic> searchEvents({
    ...
    if (type == null || type == 'session') {
      ...
      if (status != null) {
        events.addAll(sessions.where((s) => s.status.toLowerCase() == status.toLowerCase()));
      } else {
    ...
    if (type == null || type == 'payment') {
      final payments = _storageService.getAllPayments().where((payment) {
        final inRange = payment.dueDate.isAfter(startDate.subtract(const Duration(days: 1))) &&
  ...
  Map<String, dynamic> getCalendarStatistics() {
    ...
    final thisMonthPayments = _storageService.getAllPayments().where((payment) {
      return payment.dueDate.isAfter(thisMonth.subtract(const Duration(days: 1))) &&
  ...
  List<DateTime> getBusyDays({int minEvents = 3}) {
    ...
    for (DateTime day = now; day.isBefore(endDate); day = day.add(const Duration(days: 1))) {
  ...
  bool isTimeSlotAvailable(DateTime startTime, DateTime endTime) {
    ...
    for (final session in sessions) {
      // Check for overlap
      if (startTime.isBefore(session.endTime) && endTime.isAfter(session.startTime)) {
        return false;
      }
    }
  ...
  Map<String, dynamic> exportCalendarData({
    ...
    final sessions = getSessionsForDateRange(startDate, endDate);
    final payments = _storageService.getAllPayments().where((payment) {
      return payment.dueDate.isAfter(startDate.subtract(const Duration(days: 1))) &&
             payment.dueDate.isBefore(endDate.add(const Duration(days: 1)));
    }).toList();
